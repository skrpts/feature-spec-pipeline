...
context_params:
  feature_brief:
    label: "Feature Brief"
    description: "The structured feature brief produced by the decomposition stage."
    required: false
  user_stories:
    label: "User Stories"
    description: "The prioritised user stories produced by the user-story stage."
    required: false
  acceptance_criteria:
    label: "Acceptance Criteria"
    description: "The Given/When/Then acceptance criteria for each user story."
    required: false
  edge_cases:
    label: "Edge Cases"
    description: "The categorised edge-case analysis."
    required: false
  risk_assessment:
    label: "Risk Assessment"
    description: "The risk assessment for the initiative."
    required: false
...
- **Edge cases:** {{step.context.edge_cases}}
- **Risk assessment:** {{step.context.risk_assessment}}

Compile these into a single, cohesive specification document. Infer the product name from the feature brief, use "Generated by Feature Spec Pipeline" as the author, and use today's date.
...
Present the edge cases organized by severity (Critical first, then High, Medium, Low). For each edge case, include the recommended handling approach. Add a summary risk statement: "X Critical, Y High, Z Medium, W Low edge cases identified." Fold in the risk assessment: list the initiative-level risks with their likelihood, impact, and mitigation alongside the edge-case register so the reader sees both implementation edge cases and delivery risks in one place.
